...
unsigned long time_since_epoch_count()
{
...
void benchmark_fact(int n)
{
    auto mem_fact = memoize(fact);
    unsigned long start = time_since_epoch_count();
    fact(n);
    unsigned long end = time_since_epoch_count();
    unsigned long total = end - start;
    std::cout << "Run time in counts: " << total << std::endl;
}

void test_memoize_fact()
{
    benchmark_fact(50); // 1155
    benchmark_fact(50); // 731

    // TODO: huge time diff (benchmark error, copy overhead)
    //benchmark_mem_fact(50); // 17973
    //benchmark_mem_fact(50); // 2531
}
